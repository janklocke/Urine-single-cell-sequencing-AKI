...
q1 <- ggplot(URINE0_tbl, aes(x=sample_nr, y=nFeature_RNA, fill = sample_nr)) +
...
q2 <- ggplot(URINE0_tbl, aes(x=sample_nr, y=nCount_RNA, fill = sample_nr)) +
  geom_violin(scale = 'width', trim = T) +
  theme_classic() + 
  theme(axis.text.x = element_text(vjust = 0.5, hjust=1, face= "bold", size=10),
        axis.title.x = element_text(face= "bold", size=18),
...
q3 <- ggplot(URINE0_tbl, aes(x=sample_nr, y=percent.mt, fill = sample_nr)) +
  geom_violin(scale = 'width', trim = T) +
  theme_classic() + 
  theme(axis.text.x = element_text(vjust = 0.5, hjust=1, face= "bold", size=10),
        axis.title.x = element_text(face= "bold", size=18),
...
q4 <- URINE_tbl%>% group_by(sample_nr, seurat_clusters) %>%
  summarise(count = n()) %>%
  spread(seurat_clusters, count, fill = 0) %>%
  ungroup() %>%
  mutate(total_cell_count = rowSums(.[c(2:ncol(.))])) %>%
  dplyr::select(c('sample_nr', 'total_cell_count', everything())) %>%
  arrange(factor(sample_nr, levels = levels(URINE$sample_nr))) %>%
  dplyr::select(-c('total_cell_count')) %>%
  reshape2::melt(id.vars = 'sample_nr') %>%
  ggplot(aes(sample_nr, value)) +
  geom_bar(aes(fill = variable), position = 'fill', stat = 'identity') +
...
q5 <- URINE_tbl%>% group_by(sample_nr, cellgroup) %>%
  summarise(count = n()) %>%
...
q6 <- URINE_tbl%>% group_by(sample_nr, gender) %>%
  summarise(count = n()) %>%
  spread(gender, count, fill = 0) %>%
  ungroup() %>%
  mutate(total_cell_count = rowSums(.[c(2:ncol(.))])) %>%
...
q7 <- URINE_tbl %>% group_by(sample_nr, AKI_type) %>%
  summarise(count = n()) %>%
  spread(AKI_type, count, fill = 0) %>%
  ungroup() %>%
  mutate(total_cell_count = rowSums(.[c(2:ncol(.))])) %>%
...
q8 <- URINE_tbl%>% group_by(sample_nr, sampling) %>%
  summarise(count = n()) %>%
...
q9 <- URINE_tbl%>% group_by(sample_nr, gating) %>%
  summarise(count = n()) %>%
...
q10 <- URINE0_tbl%>% group_by(sample_nr, multiplet_class) %>%
  summarise(count = n()) %>%
  spread(key = multiplet_class, value = count) %>%
  mutate(perc_doublet = (doublet/(doublet + singlet)*100)) %>%
  replace_na(list(perc_doublet = 0)) %>%
  ggplot(aes(x=sample_nr, y=perc_doublet, fill = sample_nr)) +
  geom_bar(aes(fill = sample_nr), stat = 'identity') +
  theme_classic() + 
...
q11 <- URINE_tbl%>% group_by(sample_nr) %>%
  summarise(count = n()) %>%
  ggplot(aes(x=sample_nr, y=count, fill = sample_nr)) +
  geom_bar(aes(fill = sample_nr), stat = 'identity') +
  theme_classic() + 
  theme(axis.text.x = element_text(size = 10, face = "bold", vjust = 0.5, hjust=1),
        axis.text.y = element_text(vjust = 0.5, hjust=0.5), 
