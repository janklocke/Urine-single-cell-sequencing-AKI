...
pw <- comparison_plots_time(RENAL, AKI_timepoint.main, "AKI_timepoint.main", renalcellgroup, total_cell_count)+ggtitle("total")+
  comparison_plots_time(RENAL, AKI_timepoint.main, "AKI_timepoint.main", renalcellgroup, TEC_emt)+ggtitle("TEC_inj")+
...
group_bar <- function(SO) { 
  ...
  p <-ggplot(p, aes(x=AKI_timepoint.fine, y=value, fill=variable)) +
    ...
    scale_y_continuous(breaks=number_ticks(1)) +
    scale_fill_manual(values = renalcellgroup_colors[2:6], name = "renal cell type", labels=str_replace(names(renalcellgroup_colors)[2:6], "emt", "inj")) +
    theme_minimal() +
    theme(legend.position = "bottom",   
...
pw <- UC_plot_cr("P018") +  UC_plot_cr("P019") +
  UC_plot_uo("P018") +  UC_plot_uo("P019") +
  UC_plot_sc("P018") +  UC_plot_sc("P019") + 
  group_bar(RENAL_P18) + group_bar(RENAL_P19) + 
  plot_layout(design = layout2, guides = "collect") & theme(legend.position = 'right')

for (i in 1:6) { pw[[i]] <- pw[[i]] + theme(axis.text.x = element_blank(),
                                            axis.ticks.x = element_blank(),
                                            axis.title.x = element_blank() ) }
for (i in c(2,4,6,8)) { pw[[i]] <- pw[[i]] + theme(axis.text.y = element_blank(),
                                                   axis.ticks.y = element_blank(),
                                                   axis.title.y = element_blank())+ 
  NoLegend() }
for (i in c(1:7)) { pw[[i]] <- pw[[i]] + theme(axis.title.x = element_blank())}

ggsave(plot = pw , "supplfig14.png", w = 2700, h = 2200, units = "px", scale = 1, type = "cairo-png")
