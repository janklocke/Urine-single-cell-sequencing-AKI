...
# This is the R script for generating Supplemental Figure 13 in 
...
names(JOINED_Ucolors) <- levels(JOINED$renalcellgroup)[1:17]
...
## Suppl. Fig 13 A,C,D, create plots ---- 
a <- DimPlot(JOINED, reduction = "umap", group.by = "HK_nr", label = T, repel = T, pt.size = 1, label.size = 12) + 
  theme_void() + 
  scale_color_manual(labels = paste(levels(JOINED$HK_nr), levels(JOINED$HKcelltype)), values = c(HKcolors, makeTransparent("gold", 8), makeTransparent("darkturquoise", 8))) + 
  theme() +
  ggtitle("kidney") +
  theme(legend.position = "bottom",
        legend.text = element_text(size = 32),
...
b <- DimPlot(JOINED, reduction = "umap", group.by= "URO_nr", label = T, repel = T, pt.size = 1, label.size = 12) + 
  theme_void() + 
  scale_color_manual(labels = paste(levels(JOINED$URO_nr), levels(JOINED$UROcelltype)), values = c(UROcolors, makeTransparent("deeppink2", 8), makeTransparent("darkturquoise", 8))) + 
  theme() +
  ggtitle("bladder") +
  theme(legend.position = "bottom",
        legend.text = element_text(size = 32),
...
c <- DimPlot(JOINED, reduction = "umap", group.by= "rcg_nr", label = T, repel = T, pt.size = 1, label.size = 12) + 
  theme_void() + 
  scale_color_manual(labels = paste(levels(JOINED$rcg_nr), levels(JOINED$renalcellgroup)), values = c(JOINED_Ucolors, makeTransparent("deeppink2", 8), makeTransparent("gold", 8))) + 
  theme() +
...
#### compile and save ----
png("supplfig13acd.png", width=3000, height= 1200, type="cairo-png") 
a+b+c
dev.off()

# Suppl. Fig. 13C, gene expression correlation plot ----
...
URO_corr <- corr_df(URON, URO$UROcelltype)
URINE_corr <- corr_df(URINE_subset, URINE_subset$renalcellgroup)


#filter for only the highly variable genes in the datasets
HK_corr2 <- HK_corr[rownames(HK_corr) %in% c(HK@assays[["SCT"]]@var.features, URO@assays[["SCT"]]@var.features, URINE@assays[["SCT"]]@var.features),]
URO_corr2 <- URO_corr[rownames(URO_corr) %in% c(HK@assays[["SCT"]]@var.features, URO@assays[["SCT"]]@var.features, URINE@assays[["SCT"]]@var.features),]
URINE_corr2 <- URINE_corr[rownames(URINE_corr) %in% c(HK@assays[["SCT"]]@var.features, URO@assays[["SCT"]]@var.features, URINE@assays[["SCT"]]@var.features),]

# merge dataframes
MERGED_corr <- merge(HK_corr2, URINE_corr2, by = 0, all = T)
MERGED_corr <- merge(MERGED_corr, URO_corr2, by = 0, all = T)
